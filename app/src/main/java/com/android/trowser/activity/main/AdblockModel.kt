...
class AdblockModel : ActiveModel() {
    ...
    private var prefs = Trowser.instance.getSharedPreferences(Trowser.MAIN_PREFS_NAME, Context.MODE_PRIVATE)
    ...
    fun loadAdBlockList(forceReload: Boolean) = modelScope.launch {
        ...
        withContext(Dispatchers.IO) ioContext@ {
            val serializedFile = File(Trowser.instance.filesDir, SERIALIZED_LIST_FILE)
        ...
        if (!success) {
            Toast.makeText(Trowser.instance, "Error loading ad-blocker list", Toast.LENGTH_SHORT).show()
        }
